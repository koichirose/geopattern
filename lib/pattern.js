...
Pattern.prototype.fillColor = function (val) {
	return (val % 2 === 0) ? this.primaryColor : this.secondaryColor;
	// return (val % 2 === 0) ? FILL_COLOR_LIGHT : FILL_COLOR_DARK;
}

Pattern.prototype.generateBackground = function () {
	this.primaryColor = this.opts.primaryColor;
	this.secondaryColor = this.opts.secondaryColor;

	this.svg.rect(0, 0, '100%', '100%', {
		fill: color.hex2rgb(this.primaryColor)
    });
};
...
Pattern.prototype.generatePattern = function () {
	...
	if (generator) {
		...
		else if (PATTERNS.indexOf(generator) < 0) {
...
Pattern.prototype.drawInnerMosaicTile = function(svg, x, y, triangleSize, vals) {
	var triangle = buildRightTriangleShape(triangleSize);
	var opacity  = fillOpacity(vals[0]);
	var fill     = this.fillColor(vals[0]);
	var styles   = {
		stroke: STROKE_COLOR,
		'stroke-opacity': STROKE_OPACITY,
		'fill-opacity': opacity,
...
Pattern.prototype.drawOuterMosaicTile = function(svg, x, y, triangleSize, val) {
	var opacity  = fillOpacity(val);
	var fill     = this.fillColor(val);
	var triangle = buildRightTriangleShape(triangleSize);
	var styles   = {
		stroke: STROKE_COLOR,
		'stroke-opacity': STROKE_OPACITY,
...
Pattern.prototype.geoMosaicSquares = function () {
	...
	for (y = 0; y < 4; y++) {
		for (x = 0; x < 4; x++) {
			if (x % 2 === 0) {
				if (y % 2 === 0) {
					this.drawOuterMosaicTile(this.svg,
						x * triangleSize * 2,
						y * triangleSize * 2,
						triangleSize,
						hexVal(this.hash, i)
					...
					this.drawInnerMosaicTile(this.svg,
						x * triangleSize * 2,
						y * triangleSize * 2,
						triangleSize,
